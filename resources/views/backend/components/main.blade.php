 @extends('backend.layouts.admin')
 @section('title', 'Main - Admin')
 @section('content')
     <main>
         <div class="container-fluid px-4">
             <h1 class="mt-4">Main</h1>
             <ol class="breadcrumb mb-4">
                 <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">Dashboard</a></li>
                 <li class="breadcrumb-item active">Main</li>
             </ol>

             <form action="{{ route('admin.main.store') }}" method="POST" enctype="multipart/form-data">
                 @csrf
                 <div class="row">
                     <div class="form-group col-md-4 mt-3">
                         <h3>Background Image</h3>
                         <img id="bc_img_preview" style="height: 30vh; display: block"
                             src="{{ asset($main->bc_img ?? null) }}" class="img-thumbnail">
                         <input class="mt-3" type="file" id="bc_img" name="bc_img">
                     </div>
                     <div class="form-group col-md-4 mt-3">
                         <div class="mb-3">
                             <label for="title">
                                 <h4>{{ $main?->title ?? 'Title' }}</h4>
                             </label>

                             <input type="text" class="form-control" id="title" name="title"
                                 value="{{ $main?->title ?? '' }}">
                         </div>
                         <div class="mb-5">
                             <label for="sub_title">
                                 <h4>{{ $main?->sub_title ?? 'Sub Title' }}</h4>
                             </label>
                             <input type="text" class="form-control" id="sub_title" name="sub_title"
                                 value="{{ $main?->sub_title ?? '' }}">
                         </div>
                         <div>
                             <h4>Upload Resume</h4>
                             <input class="mt-2" type="file" id="resume" name="resume"
                                 value="{{ asset($main?->resume) }}">
                         </div>
                     </div>
                 </div>
                 <input type="submit" name="submit" class="btn btn-primary mt-5">

             </form>
             <script>
                 (function() {
                     var input = document.getElementById('bc_img');
                     var prev = document.getElementById('bc_img_preview');
                     if (input && prev) {
                         input.addEventListener('change', function() {
                             if (this.files && this.files[0]) {
                                 prev.src = URL.createObjectURL(this.files[0]);
                             }
                         });
                     }
                 })();
             </script>
         </div>
     </main>
 @endsection
